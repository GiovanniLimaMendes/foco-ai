...
export function initExecutive() {
  ...
  function renderConstellation() {
    const data=memory();
    const events=(data.rewards?.events || []).slice(0,42);
    $('#constellation').innerHTML=events.length
      ? events.map((event,index)=> {
          const detail=data.settings.hideConstellationDetails ? 'Progresso registrado' : `${event.label} · ${event.amount} XP`;
          return `<button class="star" style="--x:${(index*37)%92+4}%;--y:${(index*61)%80+10}%" title="${escapeHtml(detail)}" aria-label="${escapeHtml(detail)}">✦</button>`;
        }).join('')
      : '<p class="muted">Comece uma microação para acender sua primeira estrela.</p>';
  }
