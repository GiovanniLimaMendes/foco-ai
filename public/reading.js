...
export function initReading() {
  ...
  $('#readingSession').addEventListener('click', () => {
    ...
    timer = setInterval(() => { const seconds = Math.max(0, Math.ceil((end - Date.now()) / 1000)); const label = `${String(Math.floor(seconds / 60)).padStart(2,'0')}:${String(seconds % 60).padStart(2,'0')}`; $('#sessionStatus').textContent = phase === 'focus' ? `Foco no livro · ${label}` : `Pausa curta · ${label}`; if (!seconds) { clearInterval(timer); timer = null; if (phase === 'focus') window.dispatchEvent(new CustomEvent('foco:reading-session-complete',{detail:{id:crypto.randomUUID()}})); phase = phase === 'focus' ? 'break' : 'focus'; $('#readingSession').textContent = phase === 'break' ? 'Iniciar pausa de 5 min' : 'Iniciar Pomodoro'; $('#sessionStatus').textContent = phase === 'break' ? 'Pomodoro concluído. Hora de uma pausa de 5 minutos.' : 'Pausa concluída. Você pode começar outro Pomodoro se quiser.'; showToast($('#sessionStatus').textContent); } }, 250);
  });
